...
if [ -f "$SCRIPT_DIR/menu.zip" ]; then
  cp -f "$SCRIPT_DIR/menu.zip" /tmp/menu.zip
elif [ -f "./menu.zip" ]; then
  cp -f "./menu.zip" /tmp/menu.zip
else
  wget -O /tmp/menu.zip -q https://raw.githubusercontent.com/vpnmultiplus-89/MULTIPLUS_V3/main/menu.zip
fi
rm -rf menu
unzip -q /tmp/menu.zip
chmod +x menu/*
